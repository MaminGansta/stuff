		...
		while (game_running && running)
		{
			...
			while (PeekMessage(&msg, window, 0, 0, PM_REMOVE))
			{
				...
				case WM_KEYDOWN:
				{
					...
					case VK_RETURN:
					{
						input.buttons[BUTTON_ENTER].changed = input.buttons[BUTTON_DOWN].is_down != is_down;
						input.buttons[BUTTON_ENTER].is_down = is_down;
						if (is_down && input.buttons[BUTTON_ENTER].changed)
							game_running = false;
					}break;
					}
				}
...
#define MAX_SPEED 1
		// input handler
			if (input.buttons[BUTTON_UP].is_down)
			{
				...
				if (alive)
					for (int i = 0; i < 5; i++)
						if (flame.actives < 1000)
							add_particles(flame, 1, ship.pos, 0.05, vec2f(-cosf(ship.angle), -sinf(ship.angle)), PI / 2.5, float(rand() % 30 + 20) / 100, float(rand() % 130) / 100);
			}
			else
			{
				...
				for (int i = 0; i < asteroids.actives; i++)
				{
					...
					if (dist < 0.2)
					{
						...
						if (SAT_shape(ship.global, ship.faces, asteroids[i].global, asteroids[i].faces))
						{
							...
							alive = false;
							for (int i = 0; i < 500; i++)
								add_particles(flame, 1, ship.pos, 0.05, vec2f(0, 1), 2 * PI, float(rand() % 30 + 20) / 100, float(rand() % 130) / 100);
			...
			if (!alive)
			{
				if (ship_death_deleay < 0)
					game_running = false;
				ship_death_deleay -= elapsed;
			}
			...
			draw_score(score, vec2f(0.65, 0.43));
